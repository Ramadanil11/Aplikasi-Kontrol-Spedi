...
class SessionService {
  ...
  Future<ActiveSession> openSession(String deviceId) async {
    // ✅ SIMPLIFIKASI: Selalu coba ambil existing session dulu
    try {
      final existing = await getCurrentSession();
      if (existing != null && existing.deviceId == deviceId) {
        debugPrint('[SESSION] Reusing existing session: ${existing.sessionId}');
        return existing;
      }
    } catch (_) {
      // Session tidak ada atau error, lanjut buat baru
    }

    // Buat session baru
    debugPrint('[SESSION] Creating new session for device: $deviceId');
